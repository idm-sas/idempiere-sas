 ...
 */
public class MOrder extends org.compiere.model.MOrder implements DocAction
{
	/**
	 * 
	 */
	...
	public int copyLinesFrom (MOrder otherOrder, boolean counter, boolean copyASI)
	{
		...
		for (int i = 0; i < fromLines.length; i++)
		{
			...
			//sas-121
			line.set_ValueOfColumn("IsAffectPromo", false); 
	...
	{
		super.setDocAction (DocAction, forceCreation);
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if((disallowNegInv && DELIVERYRULE_Force.equals(DeliveryRule)) ||
			...
			// SAS-149 exclude override if SO Return
			if (getC_DocTypeTarget_ID() != SASSystemID.DOCTYPE_SO_RETURN_A && 
					getC_DocTypeTarget_ID() != SASSystemID.DOCTYPE_SO_RETURN_B) {
				setDeliveryRule(DELIVERYRULE_Availability);
			}
		}
	...
	public String prepareIt()
	{
		...
		String error = validateReservation();
		if (!error.isEmpty()) {
			m_processMsg = error;
			return DocAction.STATUS_Invalid;
		}
		...
		while (count != 0)
		{
			...
			for (int i = 0; i < lines.length; i++)
			{
				...
				for (MProductBOM bom : MProductBOM.getBOMLines(product))
				{
					...
					newLine.setPrice();
					newLine.set_ValueOfColumn("Source_OrderLine_ID", line.get_ID()); // SAS-161
	...
	protected boolean reserveStock (MDocType dt, MOrderLine[] lines)
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			// ISY-35 revert IDEMPIERE-2042, IDEMPIERE-2063
	...
	public boolean rejectIt()
	{
		...
		// SAS-126 Auto close quotation if rejected
		...
		if (docType.getDocBaseType().equals(MDocType.DOCBASETYPE_SalesOrder) && 
			...
			setDocAction(MOrder.DOCACTION_Complete);
			if (!processIt(MOrder.DOCACTION_Complete)) {
				log.warning("Order Process Failed: " + getDocumentNo() + " " + getProcessMsg());
				throw new IllegalStateException("SAS-126 Order Process Failed: " + getDocumentNo() + " " + getProcessMsg());
			}
			saveEx();
			
			setDocAction(MOrder.DOCACTION_Close);
			if (!processIt(MOrder.DOCACTION_Close)) {
				log.warning("Order Process Failed: " + getDocumentNo() + " " + getProcessMsg());
				throw new IllegalStateException("SAS-126 Order Process Failed: " + getDocumentNo() + " " + getProcessMsg());
	 ...
	 *	@return shipment or null
	 */
	public static MInOut createShipment(MDocType dt, Timestamp movementDate, MOrder order)
	{
//		if (log.isLoggable(Level.INFO)) log.info("For " + dt);
...
//				return null;
		...
		if (!DOCSTATUS_Completed.equals(shipment.getDocStatus()))
		{
			throw new AdempiereException ("@M_InOut_ID@: " + shipment.getProcessMsg());
//			return null;
		}
		return shipment;
	}	//	createShipment
	 ...
	 *	@return invoice or null
	 */
	public static MInvoice createInvoice (MDocType dt, org.compiere.model.MInOut shipment, Timestamp invoiceDate, MOrder order)
	{
//		if (log.isLoggable(Level.INFO)) log.info(dt.toString());
...
//			return null;
		...
		if (shipment != null)
		{
			if (!INVOICERULE_AfterDelivery.equals(order.getInvoiceRule()))
				order.setInvoiceRule(INVOICERULE_AfterDelivery);
			//
			MInOutLine[] sLines = shipment.getLines(false);
			for (int i = 0; i < sLines.length; i++)
			{
...
//					log.warning("Could not update Shipment line: " + sLine);
		...
		{
			if (!INVOICERULE_Immediate.equals(order.getInvoiceRule()))
				order.setInvoiceRule(INVOICERULE_Immediate);
			//
...
//					return null;
		...
		// Copy payment schedule from order to invoice if any
		for (MOrderPaySchedule ops : MOrderPaySchedule.getOrderPaySchedule(Env.getCtx(), order.getC_Order_ID(), 0, order.get_TrxName())) {
			MInvoicePaySchedule ips = new MInvoicePaySchedule(Env.getCtx(), 0, order.get_TrxName());
			PO.copyValues(ops, ips);
			ips.setC_Invoice_ID(invoice.getC_Invoice_ID());
			ips.setAD_Org_ID(ops.getAD_Org_ID());
			ips.setProcessing(ops.isProcessing());
...
//		counter.setBPartner(counterBP); // was set on copyFrom
	...
	{
		// ISY-99 change doc owner to creator if SO
		if (isSOTrx())
			return getCreatedBy();
		else
	...
	*/
	...
	private String validateReservation() {
		String errorMessage = "";
		MDocType docType = MDocType.get(Env.getCtx(), getC_DocTypeTarget_ID());
		if (isSOTrx() && !docType.isProposal()) {
			// SAS-27 validation availability per Org
					...
					try
					{
						...
						while (rs.next()) {
							...
							if (qtyDiff.compareTo(Env.ZERO) > 0) {
								...
								BigDecimal availableQty = DB.getSQLValueBD(get_TrxName(), sqlAvailable.toString(), 
										productID, orgID);
								...
								BigDecimal unconfirmQty = DB.getSQLValueBD(get_TrxName(), sqlUnconfirm.toString(), 
										productID, orgID);
								
								if (qtyDiff.compareTo(availableQty.subtract(unconfirmQty))> 0) {
									errorMessage = "SAS-27 Cannot reserve " + qtyDiff + " of " +
											MProduct.get(Env.getCtx(), productID).getValue() +
											" in " + MOrg.get(Env.getCtx(), orgID).getName() + 
											". Available Qty: " + availableQty.subtract(unconfirmQty);
									break;
								}
							}
						}
