 ...
 * 
 */
package id.co.databiz.sas.ui.zk.panel;
...
//		tabPanels.appendChild(desktopTabPanel);
	...
	protected void initAtpTab (int  m_M_Warehouse_ID, int m_M_Product_ID)
	{
		...
		sql = "select * " + 
				...
				" ol.m_product_id,w.m_warehouse_id " + 
				"FROM C_Order o " + 
				...
				" union " + 
				" select m.movementdate, ml.movementqty, " + 
				" productAttribute(ml.M_AttributeSetInstance_ID), ml.M_AttributeSetInstance_ID, " + 
				" dt.docbasetype, bp.name, " + 
				" dt.printname || ' ' || m.documentno , w.name, " + 
				" ml.m_product_id,w.m_warehouse_id " + 
				" from m_movement m " + 
				...
				" JOIN C_BPartner bp  ON (bp.C_BPartner_ID=m.C_BPartner_ID)" +
				" where m.docstatus = 'IP') a " + 
				" WHERE a.qtyreserved <>0"
			+ " AND a.M_Product_ID=?";
		if (m_M_Warehouse_ID != 0)
			sql += " AND a.M_Warehouse_ID=?";
		if (m_M_AttributeSetInstance_ID > 0)
			sql += " AND a.M_AttributeSetInstance_ID=?";
		sql += " ORDER BY 1";
		try
		{
			pstmt = DB.prepareStatement(sql, null);
