...
public class InvoiceHistory extends Window implements EventListener<Event>
{
	...
	private boolean dynInit()
	{
		...
		int maxrow = MSysConfig.getIntValue(AWNSysConfig.ISY_MAX_PANEL_TABLESIZE, 0, Env.getAD_Client_ID(Env.getCtx()));
		
		if (m_C_BPartner_ID == 0)
			data = queryBPartner(maxrow);		//	BPartner of Product
		else
			data = queryProduct(maxrow);		//	Product of BPartner
	 ...
	 */
	private Vector<Vector<Object>> queryProduct (int maxrow)
	{
		String sql = "SELECT p.Name,l.PriceActual,c.Iso_Code,l.PriceList,l.QtyInvoiced,"		//  1,2,3,4,5
			...
			if (maxrow > 0) {
				sql += "FETCH FIRST "+maxrow+" ROWS ONLY";		
			}
	 ...
	 */
	private Vector<Vector<Object>> queryBPartner (int maxrow)
	{
