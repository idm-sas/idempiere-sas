...
public class DPActivitiesModel {
	...
	public static boolean isShowUnprocessed() {
		Boolean isShowUnprocessed = AWNSysConfig.getBooleanValue("ISY_DP_ISSHOWUNPROCESSED", Env.getAD_Client_ID(Env.getCtx()) > 0);
		return 	isShowUnprocessed;
	}
	...
	public static int getWorkflowCount()
	{
		...
		String sql = "SELECT count(*) FROM AD_WF_Activity a "
			...
			+ " WHERE ur.IsActive='Y' AND  a.AD_WF_Responsible_ID=r.AD_WF_Responsible_ID AND r.ResponsibleType='R' AND ur.AD_User_ID=?)"	//	#4
			//
