...
public class MRequisitionLine extends org.compiere.model.MRequisitionLine
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		String sql = "SELECT COUNT(*) "
					+"FROM M_ProductPrice "
					+"WHERE M_PriceList_Version_ID=GETPRICELISTVERSION(?,?) "
					+"AND M_Product_ID=?";
		
		Integer count = DB.getSQLValue(get_TrxName(), sql, getM_Requisition().getM_PriceList_ID(), getDateRequired(), getM_Product_ID());
		
		if (count==0 && getC_Charge_ID() == 0)
		{
			throw new AdempiereException("ISY-164 : Product Not On PriceList")  ;
		}
