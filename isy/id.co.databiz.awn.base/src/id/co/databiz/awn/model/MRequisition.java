...
public class MRequisition extends org.compiere.model.MRequisition implements DocAction
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (!newRecord && (is_ValueChanged(COLUMNNAME_M_PriceList_ID) || is_ValueChanged(COLUMNNAME_DateRequired))) {
			int cnt = DB.getSQLValueEx(get_TrxName(), "SELECT COUNT(*) FROM M_RequisitionLine WHERE M_Requisition_ID=? AND M_Product_ID>0", getM_Requisition_ID());
			if (cnt > 0) {
				if (is_ValueChanged(COLUMNNAME_M_PriceList_ID)) {
					log.saveError("Error", "Price list cannot be changed because the requisition contains product lines");
					return false;
				}
				if (is_ValueChanged(COLUMNNAME_DateRequired)) {
					MPriceList pList =  MPriceList.get(getCtx(), getM_PriceList_ID(), null);
					MPriceListVersion plOld = pList.getPriceListVersion((Timestamp)get_ValueOld(COLUMNNAME_DateRequired));
					MPriceListVersion plNew = pList.getPriceListVersion((Timestamp)get_Value(COLUMNNAME_DateRequired));
					if (plNew == null || !plNew.equals(plOld)) {
						log.saveError("Error", "Date Required cannot be changed because the requisition contains product lines and the change falls in a differente price list version");
						return false; 
					}
				}
			}
	...
	public boolean closeIt()
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			sql.append("INNER JOIN M_RequisitionLine rl ON (rl.M_RequisitionLine_ID = ol.M_RequisitionLine_ID) ");
			sql.append("WHERE o.DocStatus NOT IN ('VO','RE') ");
			sql.append("AND rl.M_RequisitionLine_ID = ? ");
			finalQty = DB.getSQLValueBD(get_TrxName(), sql.toString(), line.get_ID());
