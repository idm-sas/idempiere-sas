...
//				}
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (getC_BPartner_ID() != 0 && (getC_Invoice_ID() != 0 || getC_Order_ID() != 0)) {
			...
			if (getC_Order_ID() != 0) {
				//20220110 - Ref SABA-613
				MOrder ord = new MOrder(getCtx(), getC_Order_ID(), get_TrxName());
				if (ord.getBill_BPartner_ID() != 0){
					if (ord.getBill_BPartner_ID() != getC_BPartner_ID()) {
						if (ord.getC_BPartner_ID() != getC_BPartner_ID()) {
							log.saveError("Error", Msg.parseTranslation(getCtx(), "BP different from Bill BP Order / BP different from BP Order"));
							return false;
						}					
					}
				} else if (ord.getC_BPartner_ID() != getC_BPartner_ID()) {
...
*/
	...
	public String completeIt()
	{
		...
		else
		{
			...
			//	testAllocation();
			// ISY-195
			I_C_PaymentCustom paymentCustom = POWrapper.create(this, I_C_PaymentCustom.class);
			if(!paymentCustom.isAllocateLater()){
				if(allocateIt(null)){
...
*/
	...
	protected void setDefiniteDocumentNo() {
		...
		if (dt.isOverwriteDateOnComplete()) {
			...
			setDateTrx(new Timestamp(cal.getTimeInMillis()));
	 ...
	 */
	public boolean allocateIt(Timestamp date)
	{
		if(date==null){
			date = getDateTrx();
		}
		...
		if (getC_Invoice_ID() != 0)
		{	
				return allocateInvoice(date);
		}	
		//	Invoices of a AP Payment Selection
		if (allocatePaySelection(date))
			return true;
		...
		// prevent allocation if charge
		ICPaymentAllocate allocateCustom = POWrapper.create(pAllocs[0], ICPaymentAllocate.class);
		if(allocateCustom.getC_Charge_ID()>0){
			return false;
		}
				
		MAllocationHdr alloc = new MAllocationHdr(getCtx(), false, 
				date, getC_Currency_ID(), 
				Msg.translate(getCtx(), "C_Payment_ID")	+ ": " + getDocumentNo(), 
				get_TrxName());
		alloc.setAD_Org_ID(getAD_Org_ID());
		alloc.setDateAcct(date); // in case date acct is different from datetrx in payment; IDEMPIERE-1532 tbayen
		if (!alloc.save())
		{
	 ...
	 * 	@return true if allocated
	 */
	private boolean allocateInvoice(Timestamp date)
	{
		...
		alloc.setAD_Org_ID(getAD_Org_ID());
		alloc.setDateAcct(date); // in case date acct is different from datetrx in payment
		alloc.saveEx();
		MAllocationLine aLine = null;
		if (isReceipt())
	 ...
	 * 	@return true if allocated
	 */
	private boolean allocatePaySelection(Timestamp date)
	{
		MAllocationHdr alloc = new MAllocationHdr(getCtx(), false, 
			date, getC_Currency_ID(),
	...
	public boolean voidIt()
	{
			...
			|| DOCSTATUS_NotApproved.equals(getDocStatus()) )
		{
			...
			setOverUnderAmt(Env.ZERO);
			setIsAllocated(false);
			DB.executeUpdate("UPDATE C_PaymentAllocate SET Amount = 0 WHERE C_Payment_ID = ? AND C_Charge_ID > 0", get_ID(), get_TrxName());
			//	Unlink & De-Allocate
			deAllocate(false);
		}
		else
	...
	protected StringBuilder reverse(boolean accrual) {
		...
		List<MPaymentAllocate> list = new Query(getCtx(), MPaymentAllocate.Table_Name, "C_Payment_ID = ? AND C_Charge_ID > 0", get_TrxName())
				.setParameters(new Object[]{get_ID()})
				.list();
		for(MPaymentAllocate pa : list){
			MPaymentAllocate reversalLine = new MPaymentAllocate (getCtx(), 0, get_TrxName());
			MPaymentAllocate.copyValues(pa, reversalLine);
			reversalLine.setAD_Org_ID(pa.getAD_Org_ID());
			reversalLine.setC_Payment_ID(reversal.get_ID());
			reversalLine.setAmount(pa.getAmount().negate());
			reversalLine.saveEx(get_TrxName());
		}
