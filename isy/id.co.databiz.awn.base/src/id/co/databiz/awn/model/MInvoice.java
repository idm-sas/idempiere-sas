...
public class MInvoice extends org.compiere.model.MInvoice implements DocAction
{
	...
	public void setShipment (MInOut ship)
	{
        ...
        if (ship.getM_RMA_ID() != 0)
        {
            ...
            MOrder rmaOrder = (MOrder) rma.getOriginalOrder();
            if (rmaOrder != null) {
...
//		counter.setBPartner(counterBP);// was set on copyFrom
		...
		for (int i = 0; i < counterLines.length; i++)
		{
			...
			counterLine.set_ValueNoCheck("AD_Client_ID",counter.getAD_Client_ID());
...
//        MInvoice originalInvoice = (MInvoice) rma.getOriginalInvoice();
        MInvoice originalInvoice = (MInvoice) getOriginalInvoice(rma);
	...
	public MInvoice getOriginalInvoice(MRMA rma)
    {
//       MInOut shipment = getShipment();
       ...
       else
       {
           ...
           if (invId <= 0)
           {
        	   if (MSysConfig.getValue(AWNSysConfig.ISY_GENERATE_INVOICE_CHECK_RMA_INOUTLINE, "N").equalsIgnoreCase("Y")) {
        		   String sqlinv = "select distinct il.c_invoice_id from m_rmaline rl "
        		   		+ "join c_invoiceline il on il.m_inoutline_id = rl.m_inoutline_id "
        		   		+ "where rl.m_rma_id = ? "
        		   		+ "fetch first 1 row only";
                   invId = DB.getSQLValueEx(get_TrxName(), sqlinv, rma.getM_RMA_ID());
        	   }
           }
	...
	public MOrder getOriginalOrder()
	{
		if (getM_RMA_ID() > 0)
		{
			...
			MOrder originalOrder = (MOrder) rma.getOriginalOrder();
