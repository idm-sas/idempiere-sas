...
public class MReportCube extends org.compiere.model.MReportCube {
	...
	public String update(boolean reset, boolean force) {
		...
		if ( getLastRecalculated() != null && !reset )
		{
			...
			if (log.isLoggable(Level.INFO))log.log(Level.INFO, "Selecting changed periods took:" + elapsed + "s");
			...
			if (changedPeriods != null && changedPeriods.length > 0 )
			{
				...
				if (log.isLoggable(Level.INFO))log.log(Level.INFO, "Periods requiring update: " + periodNames.toString());
			}
			else
		...
		try 
		{
			...
			if (log.isLoggable(Level.INFO))log.log(Level.INFO, "Delete sql: " + delSQL);
			start = System.currentTimeMillis();
			int deleted = DB.executeUpdateEx(delSQL, get_TrxName());
			elapsed = (System.currentTimeMillis() - start)/1000;
			result += "Deleted " + deleted + " in " + elapsed + " s;";
			
			if (log.isLoggable(Level.INFO))log.log(Level.INFO, result);
			...
			//	ISY-194
			if ( get_ValueAsBoolean("IsGLCategoryDim")){
				values.add("GL_Category_ID");
			}
			...
			if (log.isLoggable(Level.INFO))log.log(Level.INFO, sql);
			...
			if (log.isLoggable(Level.INFO))log.log(Level.INFO, insertResult);
			result += insertResult;
			...
			if (log.isLoggable(Level.INFO))log.log(Level.INFO, "Last updated: " + ts);
		...
		catch (DBException e)
		{
			...
			// nothing else to do
			if (log.isLoggable(Level.INFO))log.log(Level.INFO, getName() + " update failed:" + e.getMessage());
		}
		finally
		{
			// unlock
	...
	public String update(boolean reset, boolean force , int period) {
		...
		try 
		{
			...
			//commit agar beban server enteng isy-419
			...
			" AND y.C_Calendar_ID = ? AND f.AD_Client_ID = ? ";
			if ( getLastRecalculated() != null && !reset )
				from += "AND  p.C_Period_ID = " + period; //isy-419 hardcode c_period_id
