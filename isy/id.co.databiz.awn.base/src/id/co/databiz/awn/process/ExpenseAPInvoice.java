...
public class ExpenseAPInvoice extends SvrProcess
{
	...
	private Timestamp m_DateInvoice = null;
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			else if (name.equals("DateReport"))
			{
				...
				m_DateInvoice = (Timestamp) para[i].getParameter();
			}
			else
		...
		try
		{
			...
			while (rs.next())				//	********* Expense Line Loop
			{
				...
				if (te.getC_BPartner_ID() != old_BPartner_ID)
				{
					...
					//		ISY-277
					if(m_DateInvoice == null) {
						invoice.setDateInvoiced(te.getDateReport());
					}else {
						invoice.setDateInvoiced(m_DateInvoice);
					}
