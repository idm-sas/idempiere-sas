...
public class InOutCreateInvoice extends SvrProcess
{
	...
	{
		//recordID exist! , process run from window
		if (p_M_InOut_ID > 0) return generateInvoice(p_M_InOut_ID);
		
		//recordID = 0 , process run from info window (multi selection) 
		//https://databiz.atlassian.net/browse/ISY-411?focusedCommentId=111283
		String WhereClause = "exists (select t_selection_id from t_selection where ad_pinstance_id = ? "
				+ "	and t_selection.t_selection_id = m_inout.m_inout_id )";
		List<MInOut> ios = new Query(getCtx(), MInOut.Table_Name, WhereClause, get_TrxName())
				.setClient_ID().setParameters(new Object [] {getAD_PInstance_ID()}).list();
		String DocumentNos = "";
		for (MInOut io : ios) {
			DocumentNos = DocumentNos + generateInvoice(io.get_ID()) + " . ";
		}
		return null;
	}

	private String generateInvoice(int p_M_InOut_ID) {
