...
public class Aging extends SvrProcess
{
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			if (para[i].getParameter() == null)
				...
				p_ConversionType_ID = ((BigDecimal)para[i].getParameter()).intValue();
			else
	...
	protected String doIt() throws Exception
	{
		if (log.isLoggable(Level.INFO)) log.info("StatementDate=" + p_StatementDate + ", IsSOTrx=" + p_IsSOTrx
			...
			+ ", C_ConversionType_ID=" + p_ConversionType_ID);
		//FR 1933937
		...
		if(p_ConversionType_ID>0 && p_StatementDate!=null){
			if(p_DateAcct){
				sql.append(", currencyConvert(invoiceOpenToDate(oi.C_Invoice_ID,oi.C_InvoicePaySchedule_ID,"+dateacct+"),oi.C_Currency_ID," + SystemID.CURRRENCY_IDR + ",oi.DateInvoiced,"+p_ConversionType_ID+",oi.AD_Client_ID,oi.AD_Org_ID)");
				sql.append(", currencyConvert(invoiceOpenToDate(oi.C_Invoice_ID,oi.C_InvoicePaySchedule_ID,"+dateacct+"),oi.C_Currency_ID," + SystemID.CURRRENCY_IDR + ","+dateacct+","+p_ConversionType_ID+",oi.AD_Client_ID,oi.AD_Org_ID)");
			} else {
				sql.append(", currencyConvert(oi.OpenAmt,oi.C_Currency_ID," + SystemID.CURRRENCY_IDR + ",oi.DateInvoiced,"+p_ConversionType_ID+",oi.AD_Client_ID,oi.AD_Org_ID)");
				sql.append(", currencyConvert(oi.OpenAmt,oi.C_Currency_ID," + SystemID.CURRRENCY_IDR + ",current_date,"+p_ConversionType_ID+",oi.AD_Client_ID,oi.AD_Org_ID)");
			}
		} else {
			sql.append(", 0");	// 	23
			sql.append(", 0");	// 	24
		...
		if (p_C_DocType_ID > 0)
		{
			sql.append(" AND i.C_DocType_ID=").append(p_C_DocType_ID);
		}
		...
		try
		{
			...
			while (rs.next())
			{
				...
				BigDecimal OpenAmtOri = rs.getBigDecimal(23);
				BigDecimal OpenAmtReval = rs.getBigDecimal(24);
					...
					|| C_InvoicePaySchedule_ID != aging.getC_InvoicePaySchedule_ID())
				{
					...
					if (C_Invoice_ID > 0) {
						...
						aging.set_ValueOfColumn("C_DocType_ID", invoice.getC_DocType_ID());
						System.out.println("doctype = "+invoice.getC_DocType_ID());
//						System.out.println("Target doctype = "+invoice.getdoctyp());
						...
						aging.set_ValueOfColumn("OpenAmtOri", OpenAmtOri);
						aging.set_ValueOfColumn("OpenAmtReval", OpenAmtReval);
					}
				}
				//	Fill Buckets
