 ...
 *  @author Darwanto ~dar~
 */
public class InOutGenInv extends SvrProcess
{
	/**	Manual Selection		*/
	...
	/** Document Type	*/
	private int			p_C_DocType_ID = MOrder.Table_ID;
	...
	protected String doIt () throws Exception
	{
		...
		if (p_Selection)	//	VInOutGenInvoice
		{
			sql = "SELECT M_InOut.* FROM M_InOut, T_Selection "
				...
				+ "ORDER BY M_InOut.MovementDate, M_InOut.Documentno, M_InOut.M_Warehouse_ID, M_InOut.C_BPartner_ID, M_InOut.M_InOut_ID";
		}
		else
		{
			sql = "SELECT * FROM M_InOut o "
				+ "WHERE DocStatus IN('CO') AND IsSOTrx='Y'"
				// DocType xx10 only can be generated to Invoice ~dar~
				+ " AND SUBSTR(parse('C_DocType', o.C_DocType_ID, 1), 3, 2) = '10'";
			if (p_AD_Org_ID != 0)
				sql += " AND AD_Org_ID=?";
			...
			if( p_MovementDate_To != null )
				sql += " AND o.MovementDate <= ?";
			//
			//if( p_docTypeName.equals("O") ) {
			if( p_C_DocType_ID == MOrder.Table_ID ) {
			...
			sql	+= "ORDER BY M_InOut.Documentno, M_Warehouse_ID, C_BPartner_ID, M_InOut_ID";
		}
	//	sql += " FOR UPDATE";
		...
		//if( p_docTypeName.equals("R") )
		System.out.println(p_C_DocType_ID);
	...
	private String generate (PreparedStatement pstmt)
	{
		try
		{
			...
			while (rs.next ())
			{
				...
				p_DateInvoiced = rs.getTimestamp("MovementDate");
...
//								continue;
	...
	private void createLine (MOrder order, MInOut ship, MInOutLine sLine)
	{
		if (m_invoice == null)
		{
			...
			MDocType docType = MDocType.get(getCtx(), m_invoice.getC_DocTypeTarget_ID());
			m_invoice.set_ValueOfColumn("CreateTaxInvoice", docType.get_Value("CreateTaxInvoice")); // ISY-6
			if (!m_invoice.save())
	...
	private void completeInvoice()
	{
		...
		{
			//	{-> 001	~dar~
			MInvoiceLine[] ils = m_invoice.getLines();
			Timestamp ioDate = null;
			for( MInvoiceLine il : ils ) {
				if( il.getM_InOutLine_ID() > 0 ) {
					MInOutLine iol = (MInOutLine)il.getM_InOutLine();
					// get most recent date with positive qty
					if( ioDate == null || ((iol.getMovementQty().signum() > 0) && (ioDate.before(iol.getParent().getMovementDate())))) {
						ioDate = iol.getParent().getMovementDate();					
					}
				}
			}
			if( ioDate != null ) {
				if(!p_ConsolidateDocument){
					m_invoice.setDateInvoiced(ioDate);
					m_invoice.setDateAcct(ioDate);
				}
				m_invoice.saveEx();
			}
			//	}<- 001	~dar~
