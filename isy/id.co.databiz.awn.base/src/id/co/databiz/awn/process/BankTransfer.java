...
public class BankTransfer extends SvrProcess
{
	...
	private int 		c_DocTypeAR_Id = 0;
	private int 		c_DocTypeAP_Id = 0;
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			if (name.equals("From_C_BankAccount_ID"))
				...
				ad_OrgTrx_Id = para[i].getParameterAsInt();
			else if (name.equals("C_Project_ID"))
				c_project_id = para[i].getParameterAsInt();
			else if (name.equals("C_DocTypeAR_ID"))
				c_DocTypeAR_Id = para[i].getParameterAsInt();
			else if (name.equals("C_DocTypeAP_ID"))
				c_DocTypeAP_Id = para[i].getParameterAsInt();
			else
	...
	private void generateBankTransfer()
	{
		...
		paymentBankFrom.setAD_OrgTrx_ID(ad_OrgTrx_Id);
		paymentBankFrom.setC_Project_ID(c_project_id);
		paymentBankFrom.setC_DocType_ID(c_DocTypeAP_Id); //APP
		...
		paymentBankTo.setAD_OrgTrx_ID(ad_OrgTrx_Id);
		paymentBankTo.setC_Project_ID(c_project_id);
		paymentBankTo.setC_DocType_ID(c_DocTypeAR_Id); //ARR
		...
		//	ISY-106
		paymentBankFrom.set_ValueOfColumn("C_PaymentTransfer_ID", paymentBankTo.getC_Payment_ID());
		paymentBankFrom.saveEx();
		paymentBankTo.set_ValueOfColumn("C_PaymentTransfer_ID", paymentBankFrom.getC_Payment_ID());
		paymentBankTo.saveEx();
