...
public class InOutGenerate extends SvrProcess
{
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			//	DocAction check - default is None (Document left in draft state)
			if (!DocAction.ACTION_Complete.equals(p_docAction) && !DocAction.ACTION_Prepare.equals(p_docAction))
	...
	private String generate (PreparedStatement pstmt)
	{
		...
		try
		{
			...
			while (rs.next ())		//	Order
			{
				...
				// Exclude Unpaid if After Receipt
//				if (MOrder.DELIVERYRULE_AfterReceipt.equals(order.getDeliveryRule())) { --IDM 11 Ganti Nama Menjadi DELIVERYRULE_AfterPayment
				if (order.getDeliveryRule().equals("R")) {
					where.append(" AND EXISTS (SELECT 1 FROM C_Payment p");
					where.append(" LEFT JOIN C_Invoice i ON (i.C_Invoice_ID = p.C_Invoice_ID)");
					where.append(" WHERE COALESCE(i.C_Order_ID,p.C_Order_ID) = C_OrderLine.C_Order_ID AND p.DocStatus IN('CO','CL'))");
				}
				...
				for (int i = 0; i < lines.length; i++)
				{
					...
					// After Receipt
//					else if (fullLine && MOrder.DELIVERYRULE_AfterReceipt.equals(order.getDeliveryRule())) --IDM 11 Ganti Nama Menjadi DELIVERYRULE_AfterPayment
					else if (fullLine && order.getDeliveryRule().equals("R"))

					{
						if (log.isLoggable(Level.FINE)) log.fine("AfterReceipt - OnHand=" + onHand 
							+ " (Unconfirmed=" + unconfirmedShippedQty
							+ ", ToDeliver=" + toDeliver + " - " + line);
						//	
						createLine (order, line, toDeliver, storages, false);
