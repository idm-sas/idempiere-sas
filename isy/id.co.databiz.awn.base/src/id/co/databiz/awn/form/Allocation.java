...
public class Allocation
{
	...
	public int         	m_C_BPartnerRelation_ID = 0;
...
//	private int			i_multiplier = 10;
	...
	public void dynInit() throws Exception
	{
		...
		m_AD_OrgTrx_ID = Env.getContextAsInt(Env.getCtx(), "AD_OrgTrx_ID");
		m_C_DocType_ID= MDocType.getDocType("CMA");
		m_C_BPartnerRelation_ID = Env.getContextAsInt(Env.getCtx(), "C_BPartnerRelation_ID");
	}
	...
	{		
		...
		if (m_AD_OrgTrx_ID != 0 )
			sql.append(" AND p.AD_OrgTrx_ID=" + m_AD_OrgTrx_ID);
		...
		try
		{
			...
			pstmt.setTimestamp(2, TimeUtil.getDay((Timestamp)date));
			pstmt.setInt(3, m_C_Currency_ID);
			pstmt.setTimestamp(4, TimeUtil.getDay((Timestamp)date));
...
//		paymentTable.setColumnClass(i++, BigDecimal.class, true);      	//  8-Multiplier
	...
	public Vector<Vector<Object>> getInvoiceData(boolean isMultiCurrency, Object date, IMiniTable invoiceTable)
	{
		...
		StringBuilder sql = new StringBuilder("SELECT i.DateInvoiced,i.DocumentNo,i.C_Invoice_ID," //  1..3
			...
			+ " AND (i.C_BPartner_ID IN (SELECT bpr.C_BPartnerRelation_ID FROM C_BP_Relation bpr WHERE bpr.C_BPartner_ID=?");
		if (m_C_BPartnerRelation_ID != 0 )
			sql.append(" AND bpr.C_BPartnerRelation_ID=" + m_C_BPartnerRelation_ID);
		sql.append(") OR i.C_BPartner_ID=?)");         //  #7, #8 	ISY-87
		//if (!isMultiCurrency)
			sql.append(" AND i.C_Currency_ID=?");                                   //  #9
		if (m_AD_Org_ID != 0 ) 
			sql.append(" AND i.AD_Org_ID=" + m_AD_Org_ID);
		if (m_AD_OrgTrx_ID != 0 )
			sql.append(" AND i.AD_OrgTrx_ID=" + m_AD_OrgTrx_ID);
		...
		try
		{
			...
			pstmt.setInt(3, m_C_Currency_ID);
			pstmt.setTimestamp(4, TimeUtil.getDay((Timestamp)date));
			pstmt.setTimestamp(5, TimeUtil.getDay((Timestamp)date));
			pstmt.setInt(6, m_C_Currency_ID);
...
//		i_multiplier = isMultiCurrency ? 10 : 8;
	...
	public MAllocationHdr saveData(int m_WindowNo, Object date, IMiniTable payment, IMiniTable invoice, String trxName)
	{
		...
		// ISY-151
		// 20220315 override org from invoice
//		alloc.setAD_Org_ID(AD_Org_ID);
		...
		for (int i = 0; i < iRows; i++)
		{
			...
			if (((Boolean)invoice.getValueAt(i, 0)).booleanValue())
			{
				...
				alloc.setAD_Org_ID(mInvoice.getAD_Org_ID());
