 ...
 *
 */
public class DocAllocationHdr extends Doc_AllocationHdr
{
	/**
	 *  Constructor
	 ...
	 * 	@param trxName trx
	 */
	public DocAllocationHdr (MAcctSchema as, ResultSet rs, String trxName)
	{
		super (as, rs, trxName);
	}   //  Doc_Allocation
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		//ISY-151 20220309 init variable buat dapatkan nilai2 dari record sebelumnya
		int AD_OrgTrx_ID = 0;
		int C_Project_ID = 0;
		int C_Campaign_ID = 0; 
		int C_Activity_ID = 0;
...
//					ISY-151
					...
					MInvoice invoiceCharge = null;
					MPayment paymentCharge = null;
					for(int j=0; j<p_lines.length; j++){
						DocLine_Allocation docLine = (DocLine_Allocation)p_lines[j];
						if(docLine.getC_Invoice_ID()>0){
							invoiceCharge = new MInvoice(getCtx(), docLine.getC_Invoice_ID(), getTrxName());
							if(invoiceCharge.getTotalLines().compareTo(invAmt) > 0 || invoiceCharge.getC_Charge_ID() != SystemID.CURRRENCY_IDR ||
									(invoiceCharge.getTotalLines().compareTo(invAmt)==0 && (dateInvoiced == null || invoiceCharge.getDateInvoiced().before(dateInvoiced)))){
								dateInvoiced = invoiceCharge.getDateInvoiced();
								invAmt = invoiceCharge.getTotalLines();
								fl.setAD_OrgTrx_ID(invoiceCharge.getAD_OrgTrx_ID());
								fl.setC_Project_ID(invoiceCharge.getC_Project_ID());
								fl.setC_Campaign_ID(invoiceCharge.getC_Campaign_ID());
								fl.setUser1_ID(invoiceCharge.getUser1_ID());
								fl.setC_Activity_ID(invoiceCharge.getC_Activity_ID());
								fl.setUser2_ID(invoiceCharge.getUser2_ID());
							}
						}else if(docLine.getC_Payment_ID()>0 && invoiceCharge==null){
							paymentCharge = new MPayment(getCtx(), docLine.getC_Payment_ID(), getTrxName());
							if(paymentCharge.getPayAmt().compareTo(payAmt) > 0 ||
									(paymentCharge.getPayAmt().compareTo(payAmt)==0 && (datePayment == null || paymentCharge.getDateTrx().before(datePayment)))){
								datePayment = paymentCharge.getDateTrx();
								payAmt = paymentCharge.getPayAmt();
								fl.setAD_OrgTrx_ID(paymentCharge.getAD_OrgTrx_ID());
								fl.setC_Project_ID(paymentCharge.getC_Project_ID());
								fl.setC_Campaign_ID(paymentCharge.getC_Campaign_ID());
								fl.setUser1_ID(paymentCharge.getUser1_ID());
								fl.setC_Activity_ID(paymentCharge.getC_Activity_ID());
								fl.setUser2_ID(paymentCharge.getUser2_ID());
							}
						}
					}
					//ISY-151 20220309
					AD_OrgTrx_ID = invoiceCharge == null ? paymentCharge.getAD_OrgTrx_ID():invoiceCharge.getAD_OrgTrx_ID();
					C_Project_ID = invoiceCharge == null ? paymentCharge.getC_Project_ID():invoiceCharge.getC_Project_ID();
					C_Campaign_ID = invoiceCharge == null ? paymentCharge.getC_Campaign_ID():invoiceCharge.getC_Campaign_ID();
					C_Activity_ID = invoiceCharge == null ? paymentCharge.getC_Activity_ID():invoiceCharge.getC_Activity_ID();
					User1_ID = invoiceCharge == null ? paymentCharge.getUser1_ID():invoiceCharge.getUser1_ID();
					C_BPartner_ID = invoiceCharge == null ? paymentCharge.getC_BPartner_ID():invoiceCharge.getC_BPartner_ID();
				}
				//	Payment Only
				else if (line.getC_Invoice_ID() == 0 && line.getC_Payment_ID() != 0)
				{
					...
					if (fl != null && payment != null) {
						...
						fl.setC_Campaign_ID(payment.getC_Campaign_ID());
						fl.setC_Activity_ID(payment.getC_Activity_ID());
						fl.setUser1_ID(payment.getUser1_ID());
						fl.setUser2_ID(payment.getUser2_ID());
						allocPayAccounted = allocPayAccounted.add(fl.getAcctBalance());
						//ISY-151 20220309
						AD_OrgTrx_ID = payment.getAD_OrgTrx_ID();
						C_Project_ID = payment.getC_Project_ID();
						C_Campaign_ID = payment.getC_Campaign_ID();
						C_Activity_ID = payment.getC_Activity_ID();
						User1_ID = payment.getUser1_ID();
			...
			else if (invoice.isSOTrx())
			{
				...
				if (as.isAccrual())
				{
					...
					if (fl != null && invoice != null) {
						...
						fl.setC_Campaign_ID(invoice.getC_Campaign_ID());
						fl.setC_Activity_ID(invoice.getC_Activity_ID());
						fl.setUser1_ID(invoice.getUser1_ID());
						fl.setUser2_ID(invoice.getUser2_ID());
					}						
			...
			//ISY-151 20220309
			if (invoice != null) {
				AD_OrgTrx_ID = invoice.getAD_OrgTrx_ID();
				C_Project_ID = invoice.getC_Project_ID();
				C_Campaign_ID = invoice.getC_Campaign_ID();
				C_Activity_ID = invoice.getC_Activity_ID();
				User1_ID = invoice.getUser1_ID();
		...
		if (getC_Currency_ID() != as.getC_Currency_ID())
			balanceAccounting(as, fact, AD_OrgTrx_ID, C_Project_ID, C_Campaign_ID, C_Activity_ID, User1_ID, C_BPartner_ID);
...
//			ISY-151
			BigDecimal invAmt = Env.ZERO;
			Date dateInvoiced = null;
			MInvoice invoiceRGL = null;
			for(int j=0; j<p_lines.length; j++){
				DocLine_Allocation docLine = (DocLine_Allocation)p_lines[j];
				if(docLine.getC_Invoice_ID()>0){
					invoiceRGL = new MInvoice(getCtx(), docLine.getC_Invoice_ID(), getTrxName());
					if(invoiceRGL.getTotalLines().compareTo(invAmt) > 0 || 
							(invoiceRGL.getTotalLines().compareTo(invAmt)==0 && (dateInvoiced == null || invoiceRGL.getDateInvoiced().before(dateInvoiced)))){
						dateInvoiced = invoiceRGL.getDateInvoiced();
						invAmt = invoiceRGL.getTotalLines();
						fl.setAD_OrgTrx_ID(invoiceRGL.getAD_OrgTrx_ID());
						fl.setC_Project_ID(invoiceRGL.getC_Project_ID());
						fl.setC_Campaign_ID(invoiceRGL.getC_Campaign_ID());
						fl.setUser1_ID(invoiceRGL.getUser1_ID());
						fl.setC_Activity_ID(invoiceRGL.getC_Activity_ID());
						fl.setUser2_ID(invoiceRGL.getUser2_ID());
					}
				}
			}
		}
	 ...
	 * @return
	 */
	private FactLine balanceAccounting(MAcctSchema as, Fact fact, int AD_OrgTrx_ID, int C_Project_ID, int C_Campaign_ID, int C_Activity_ID, int User1_ID, int C_BPartner_ID)
	{
		FactLine line = null;
		if (!fact.isAcctBalanced())
		{
			...
			line.setC_Project_ID(C_Project_ID);
			line.setC_Campaign_ID(C_Campaign_ID);
