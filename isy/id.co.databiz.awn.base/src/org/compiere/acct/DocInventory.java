 ...
 */
public class DocInventory extends Doc_Inventory
{
	 ...
	 * 	@param trxName trx
	 */
	public DocInventory (MAcctSchema as, ResultSet rs, String trxName)
	{
		super (as, rs, trxName);
...
//				BigDecimal currentQty = cost.getCurrentQty();
				MInventoryLine inventoryLine = new MInventoryLine(getCtx(), line.get_ID(), getTrxName());
				BigDecimal currentQty = Env.ZERO;
				if (inventoryLine.get_Value("QtyCostAdjustment") != null) {
					currentQty = (BigDecimal) inventoryLine.get_Value("QtyCostAdjustment");
				}
			...
			{		
				...
				if (dr != null)
				{
					...
					if (cr != null)
					{
						...
						cr.setAD_OrgTrx_ID(inventory.getAD_OrgTrx_ID());
					}
					dr.setAD_OrgTrx_ID(inventory.getAD_OrgTrx_ID());
				}
			}
