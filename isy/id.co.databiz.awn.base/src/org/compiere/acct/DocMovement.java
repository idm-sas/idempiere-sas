 ...
 */
public class DocMovement extends Doc_Movement
{
	 ...
	 * 	@param trxName trx
	 */
	public DocMovement (MAcctSchema as, ResultSet rs, String trxName)
	{
		super (as, rs, trxName);
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			MLocator locator = new MLocator(this.getCtx(), line.getM_Locator_ID(), this.getTrxName());
			MLocator locatorTo = new MLocator(this.getCtx(), line.getM_LocatorTo_ID(), this.getTrxName());
			MWarehouse warehouse = new MWarehouse(this.getCtx(), locator.getM_Warehouse_ID(), this.getTrxName());
			MWarehouse warehouseTo = new MWarehouse(this.getCtx(), locatorTo.getM_Warehouse_ID(), this.getTrxName());
            AD_OrgTrx_ID = locator.get_ValueAsInt("AD_OrgTrx_ID");
            if (AD_OrgTrx_ID <= 0) {
                AD_OrgTrx_ID = warehouse.get_ValueAsInt("AD_OrgTrx_ID");
            }
            AD_OrgTrx_ID_To = locatorTo.get_ValueAsInt("AD_OrgTrx_ID");
            if (AD_OrgTrx_ID_To <= 0) {
                AD_OrgTrx_ID_To = warehouseTo.get_ValueAsInt("AD_OrgTrx_ID");
            }
			...
			dr.setAD_OrgTrx_ID(AD_OrgTrx_ID);
			if (isReversal(line))
			{
			...
			cr.setAD_OrgTrx_ID(AD_OrgTrx_ID_To);
			if (isReversal(line))
			{
