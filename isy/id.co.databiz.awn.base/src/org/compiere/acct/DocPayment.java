 ...
 */
public class DocPayment extends Doc_Payment
{
	/**
	 *  Constructor
	 ...
	 * 	@param trxName trx
	 */
	public DocPayment (MAcctSchema as, ResultSet rs, String trxName)
	{
		super (as, rs, trxName);
	}	//	Doc_Payment
	...
	/**	Contained Doc Lines			*/
	protected List<DocLine>			p_lines;
	...
	protected String loadDocumentDetails ()
	{
		...
		p_lines = loadLines(pay);
		
		return null;
	}   //  loadDocumentDetails
	
	/**
	 *	Load Payment Allocate
	 *	@param payment payment
	 *  @return DocLine Array
	 */
	private List<DocLine> loadLines (MPayment payment) {
		ArrayList<DocLine> list = new ArrayList<DocLine>();
		//
		List<MPaymentAllocate> allocateList = new Query(getCtx(), MPaymentAllocate.Table_Name, 
				"C_Charge_ID > 0 AND C_Payment_ID = ?", getTrxName())
			.setParameters(new Object[]{payment.get_ID()})
			.list();
		for (MPaymentAllocate line : allocateList) {
			DocLine docLine = new DocLine(line, this);
			
			if (payment.isReceipt()) {
				docLine.setAmount(Env.ZERO, line.getAmount());
			} else {
				docLine.setAmount(line.getAmount(), Env.ZERO);
			}
			//
		...
		if (getDocumentType().equals(DOCTYPE_ARReceipt))
		{
			...
			if(p_lines.size() > 0){
				for(DocLine docLine : p_lines){
					acct = docLine.getChargeAccount(as, Env.ZERO);
					fl = fact.createLine(docLine, acct,
							getC_Currency_ID(), null, docLine.getAmtSourceCr());
					if (fl != null && AD_Org_ID != 0
		...
		else if (getDocumentType().equals(DOCTYPE_APPayment))
		{
			...
			if(p_lines.size() > 0){
				for(DocLine docLine : p_lines){
					acct = docLine.getChargeAccount(as, Env.ZERO);
					fl = fact.createLine(docLine, acct,
							getC_Currency_ID(),docLine.getAmtSourceDr(),null);
					if (fl != null && AD_Org_ID != 0
							&& getC_Charge_ID() == 0)		//	don't overwrite charge
							fl.setAD_Org_ID(AD_Org_ID);
				}
