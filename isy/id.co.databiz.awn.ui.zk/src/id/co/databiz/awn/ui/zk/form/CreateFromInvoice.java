...
public abstract class CreateFromInvoice extends CreateFrom
{
	...
	protected Vector<Vector<Object>> getShipmentData(int M_InOut_ID)
	{
		...
		sql.append(" l.QtyEntered/l.MovementQty,"
			...
			+ " l.M_Product_ID, p.Value||'-'||p.Name, po.VendorProductNo, l.M_InOutLine_ID, l.Line,"        //  5..9
			+ " l.C_OrderLine_ID " //  10
			+ " FROM M_InOutLine l "
			);
		...
		sql.append(" LEFT OUTER JOIN M_Product_PO po ON (l.M_Product_ID = po.M_Product_ID AND io.C_BPartner_ID = po.C_BPartner_ID)")
			...
			.append("GROUP BY l.MovementQty, l.QtyEntered/l.MovementQty, "
				...
				+ "l.M_Product_ID, p.Value||'-'||p.Name, po.VendorProductNo, l.M_InOutLine_ID, l.Line, l.C_OrderLine_ID ");
		if(!isSOTrx)
	...
	protected Vector<Vector<Object>> getRMAData(int M_RMA_ID)
	{
	    ...
	    sqlStmt.append("SELECT rl.M_RMALine_ID, rl.line, rl.Qty - COALESCE(rl.QtyInvoiced, 0), iol.M_Product_ID, p.Value||'-'||p.Name, uom.C_UOM_ID, COALESCE(uom.UOMSymbol,uom.Name) ");
...
//						}
	...
	protected Vector<Vector<Object>> getOrderData (int C_Order_ID, boolean forInvoice)
	{
		...
		StringBuilder sql = new StringBuilder("SELECT "
			...
			+ " l.C_UOM_ID,COALESCE(uom.UOMSymbol,uom.Name),"			//	3..4
			+ " COALESCE(l.M_Product_ID,0),COALESCE(p.Value||'-'||p.Name,c.Name),po.VendorProductNo,"	//	5..7
		...
		sql.append(" WHERE l.C_Order_ID=? "			//	#1
			...
			+ "l.C_UOM_ID,COALESCE(uom.UOMSymbol,uom.Name),po.VendorProductNo, "
				+ "l.M_Product_ID,COALESCE(p.Value||'-'||p.Name,c.Name), l.Line,l.C_OrderLine_ID "
			+ "ORDER BY l.Line");
		//
