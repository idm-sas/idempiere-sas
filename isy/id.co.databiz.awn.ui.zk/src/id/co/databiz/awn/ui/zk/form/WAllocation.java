...
import id.co.databiz.awn.form.Allocation;
	...
	implements IFormController, EventListener<Event>, WTableModelListener, ValueChangeListener
{
	...
	private Label organizationTrxLabel = new Label();
	private WTableDirEditor organizationTrxPick;
	private Label bPartnerRelationLabel = new Label();
	private WSearchEditor bPartnerRelationSearch = null;
	...
	private void zkInit() throws Exception
	{
		...
		organizationTrxLabel.setText(Msg.translate(Env.getCtx(), "AD_OrgTrx_ID"));
		bPartnerRelationLabel.setText(Msg.translate(Env.getCtx(), "C_BPartnerRelation_ID"));
	...
	protected void layoutParameterAndSummary() {
		...
		row.appendCellChild(organizationTrxLabel.rightAlign());
		ZKUpdateUtil.setHflex(organizationTrxPick.getComponent(), "true");
		row.appendCellChild(organizationTrxPick.getComponent(),1);
		...
		row = rows.newRow();
		row.appendCellChild(bPartnerRelationLabel.rightAlign());
		bPartnerRelationSearch.getComponent().setHflex("true");
		row.appendCellChild(bPartnerRelationSearch.getComponent(),2);
	...
	public void dynInit() throws Exception
	{
		...
		MLookup lookupOrg = MLookupFactory.get(ctx, 
				form.getWindowNo(), 
		...
		//ISY-87
		//MLookup lookupOrg = MLookupFactory.get(Env.getCtx(), form.getWindowNo(), 0, AD_Column_ID, DisplayType.TableDir);
		MLookup lookupOrgTrx = MLookupFactory.get(ctx, 
				form.getWindowNo(), 
				MColumn.getColumn_ID(MOrg.Table_Name,MOrg.COLUMNNAME_AD_Org_ID), 
				DisplayType.TableDir,
				Env.getLanguage(ctx), 
				...
				322,
				false,
				"IsOrgTrxDim='Y' AND IsSummary='N' AND AD_Org_ID <> 0");
		organizationTrxPick = new WTableDirEditor("AD_OrgTrx_ID", true, false, true, lookupOrgTrx);
		organizationTrxPick.setValue(Env.getContextAsInt(Env.getCtx(), "AD_OrgTrx_ID"));
		organizationTrxPick.addValueChangeListener(this);
		...
		//  BPartnerRelation
		AD_Column_ID = SystemID.COLUMN_C_BP_RELATION_C_BPARTNERRELATION_ID;        //  C_Invoice.C_BPartner_ID
		MLookup lookupBPR = MLookupFactory.get (Env.getCtx(), form.getWindowNo(), 0, AD_Column_ID, DisplayType.Search);
		bPartnerRelationSearch = new WSearchEditor("C_BPartnerRelation_ID", true, false, true, lookupBPR);
		bPartnerRelationSearch.addValueChangeListener(this);
	...
	public void onEvent(Event e)
	{
		...
		//	ISY-121 Disable Charge When multiCurrency Selected
		if (multiCurrency.isSelected()){
			m_C_Charge_ID = 0;
			chargePick.setVisible(false);
			chargeLabel.setVisible(false);
		}else{
			chargePick.setVisible(true);
	...
	public void valueChange (ValueChangeEvent e)
	{
		...
		{
			m_AD_OrgTrx_ID = ((Integer) value).intValue();
		...
		//  BPartnerRelation
		if (name.equals("C_BPartnerRelation_ID"))
		{
			bPartnerRelationSearch.setValue(value);
			m_C_BPartnerRelation_ID = ((Integer)value).intValue();
			loadBPartner();
		}
