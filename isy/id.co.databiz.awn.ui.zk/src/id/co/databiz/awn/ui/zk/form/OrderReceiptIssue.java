...
public class OrderReceiptIssue extends GenForm {
	...
	public void configureMiniTable(IMiniTable issue) {
		...
		issue.setColumnClass(6, BigDecimal.class, true,
				Msg.translate(Env.getCtx(), "QtyRequiered"));
	...
	public void executeQuery(IMiniTable issue) {
		...
		try {
			...
			while (rs.next()) {
				...
				BigDecimal qtyRequiered = rs.getBigDecimal(8);
				...
				// ... 5 - ASI
				issue.setValueAt(qtyRequiered, row, 6); // QtyRequiered
				...
				if (componentType
					...
					// If the there is product on hand and product is requiered
					// the product should be selected
					id.setSelected(qtyOnHand.signum() > 0
							&& qtyRequiered.signum() > 0);
					issue.setValueAt(id, row, 0); // PP_OrderBOMLine_ID
					...
					if (isQtyPercentage) {
						...
						if (isBackflush()) { // Is Backflush - Calculate
							...
							if (qtyRequiered.signum() == 0
								...
								if (qtyRequiered.subtract(qtyDelivered).signum() < 0
										| componentToDeliverQty.signum() == 0)
									componentToDeliverQty = qtyRequiered
							...
							if (componentToDeliverQty.signum() != 0) {
								...
								// ...instead of toDeliverQty, qtyRequiered
								// should be used!
								// componentQtyReq =
	...
	private int lotes(int row, IDColumn id, int Warehouse_ID, int M_Product_ID,
			BigDecimal qtyRequiered, BigDecimal qtyToDelivery, IMiniTable issue) {
		int linesNo = 0;
		BigDecimal qtyRequieredActual = qtyRequiered;
		...
		try {
			...
			while (rs.next()) {
				...
				// Qty Requiered:
				if (qtyRequieredActual.compareTo(qtyOnHand) < 0) {
					issue.setValueAt(
							qtyRequieredActual.signum() > 0 ? qtyRequieredActual
									: Env.ZERO, row, 6);
				} else {
					issue.setValueAt(qtyOnHand, row, 6);
				}
				qtyRequieredActual = qtyRequieredActual.subtract(qtyOnHand);
