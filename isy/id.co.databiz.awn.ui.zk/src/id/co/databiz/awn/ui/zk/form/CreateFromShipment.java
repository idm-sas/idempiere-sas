...
public abstract class CreateFromShipment extends CreateFrom 
{
	...
	protected Vector<Vector<Object>> getOrderData (int C_Order_ID, boolean forInvoice)
	{
		...
		sql.append(" WHERE l.C_Order_ID=? "			//	#1
				...
				+ "l.M_Product_ID,COALESCE(p.Name,c.Name), l.Line,l.C_OrderLine_ID,p.Value,l.Description "
				+ "ORDER BY l.Line");
		//
		...
		try
		{
			...
			while (rs.next())
			{
				...
				// Add product
				line.add(rs.getString("ProductValue"));				// 12-Product Key
				...
				line.add(rs.getString("Description"));  //  9-Description
				data.add(line);
			}
		}
		catch (SQLException e)
	...
	protected Vector<Vector<Object>> getRMAData(int M_RMA_ID)
	{
	    ...
	    sqlStmt.append("SELECT rl.M_RMALine_ID, rl.line, rl.Qty - rl.QtyDelivered -COALESCE((SELECT SUM(iol.MovementQty) FROM M_InOutLine iol JOIN M_InOut io ON iol.M_InOut_ID=io.M_InOut_ID WHERE rl.M_RMALine_ID=iol.M_RMALine_ID AND io.Processed='N'),0), p.M_Product_ID, COALESCE(p.Name, c.Name), uom.C_UOM_ID, COALESCE(uom.UOMSymbol,uom.Name), p.Value AS ProductValue, rl.Description ");
	    ...
	    if (MSysConfig.getValue(AWNSysConfig.ISY_CUSTOMER_RETURN_CREATE_LINES, "N").equals("Y")) {
	    	sqlStmt.append("LEFT OUTER JOIN C_UOM uom ON (uom.C_UOM_ID=COALESCE(p.C_UOM_ID,iol.C_UOM_ID)) ");
	    }
	          
	    else if (Env.isBaseLanguage(Env.getCtx(), "C_UOM"))
        {
	    ...
	    sqlStmt.append("SELECT rl.M_RMALine_ID, rl.line, rl.Qty - rl.QtyDelivered -COALESCE((SELECT SUM(iol.MovementQty) FROM M_InOutLine iol JOIN M_InOut io ON iol.M_InOut_ID=io.M_InOut_ID WHERE rl.M_RMALine_ID=iol.M_RMALine_ID AND io.Processed='N'),0), p.M_Product_ID, p.Name, uom.C_UOM_ID, COALESCE(uom.UOMSymbol,uom.Name), p.Value AS ProductValue, rl.Description ");
	    ...
	    sqlStmt.append("SELECT rl.M_RMALine_ID, rl.line, rl.Qty - rl.QtyDelivered -COALESCE((SELECT SUM(iol.MovementQty) FROM M_InOutLine iol JOIN M_InOut io ON iol.M_InOut_ID=io.M_InOut_ID WHERE rl.M_RMALine_ID=iol.M_RMALine_ID AND io.Processed='N'),0), 0, c.Name, uom.C_UOM_ID, COALESCE(uom.UOMSymbol,uom.Name), NULL AS ProductValue, rl.Description ");
	    ...
	    try
	    {
	        ...
	        while (rs.next())
            {
	            ...
	            line.add(rs.getString("ProductValue")); // ProductValue
				line.add(pp); // 4-Product
	...
	protected Vector<Vector<Object>> getInvoiceData(int C_Invoice_ID)
	{
		...
		.append("GROUP BY l.QtyInvoiced,l.QtyEntered/l.QtyInvoiced,"
				...
				+ "l.M_Product_ID,p.Name, po.VendorProductNo, l.C_InvoiceLine_ID,l.Line,l.C_OrderLine_ID,p.Value,l.Description ")
		...
		try
		{
			...
			while (rs.next())
			{
				...
				// Add locator
				line.add(getLocatorKeyNamePair(rs.getInt(5))); // 3-Locator
				line.add(rs.getString("ProductValue")); // 13-Product Key
				pp = new KeyNamePair(rs.getInt(7), rs.getString(8));
				line.add(pp); // 4-Product
				line.add(rs.getString(9));				// 5-VendorProductNo
				...
				line.add(rs.getString("Description")); // 14-Description
				data.add(line);
			}
		}
		catch (SQLException e)
	...
	protected void configureMiniTable (IMiniTable miniTable)
	{
		...
		miniTable.setColumnClass(10, String.class, true); // Description
...
//				iol.setM_Product_ID(M_Product_ID, C_UOM_ID);	//	Line UOM
				iol.setM_Product_ID(M_Product_ID); //UOM nya di set berdasarkan jenis dokumen saja
	...
	protected Vector<String> getOISColumnNames()
	{
	    ...
	    columnNames.add(Msg.translate(Env.getCtx(), "ProductValue"));
