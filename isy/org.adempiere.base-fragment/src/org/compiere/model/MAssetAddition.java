...
//	private boolean m_confirmed_AssetValues = false;
	...
	public static MAssetAddition createAsset(MMatchInv match)
	{
		...
		asset.dump();
		//@win add
		MAssetGroupAcct assetgrpacct = (MAssetGroupAcct) MAssetGroupAcct.forA_Asset_Group_ID(asset.getCtx(), asset.getA_Asset_Group_ID(), assetAdd.getPostingType());
	...
	private MAssetAddition (MProject project)
	{
		...
		set_ValueNoCheck("DeltaUseLifeYears",I_ZERO);
		set_ValueNoCheck("DeltaUseLifeYears_F",I_ZERO);
		setC_DocType_ID();
	...
	private MAssetAddition (MIFixedAsset ifa)
	{
		...
		set_ValueNoCheck("A_Accumulated_Depr_Adjust",isAccmDeprAdjust);
		set_ValueNoCheck("A_Period_Start",ifa.getA_Current_Period());
		
		set_ValueNoCheck("A_Accumulated_Depr",ifa.getA_Accumulated_Depr());
		set_ValueNoCheck("A_Accumulated_Depr_F",ifa.getA_Accumulated_Depr_F());
		set_ValueNoCheck("DeltaUseLifeYears",(int)(ifa.getUseLifeMonths() / 12));
		set_ValueNoCheck("DeltaUseLifeYears_F",(int)(ifa.getUseLifeMonths_F() / 12));
	...
	public String completeIt() 
	{
		...
		*/
		...
		assetwk.adjustUseLife(get_ValueAsInt("DeltaUseLifeYears"), get_ValueAsInt("DeltaUseLifeYears_F"), isA_CreateAsset()); // reset if isA_CreateAsset
		...
		if (isA_CreateAsset() && !get_ValueAsBoolean("A_Accumulated_Depr_Adjust"))
		{
		...
		if (get_ValueAsBoolean("A_Accumulated_Depr_Adjust"))
		{
			if (isA_CreateAsset() && get_ValueAsBoolean("A_Accumulated_Depr_Adjust"))
			{
	...
	private void reverseIt(boolean isReActivate)
	{
		...
		assetwk.adjustUseLife(0 - get_ValueAsInt("DeltaUseLifeYears"), 0 - get_ValueAsInt("DeltaUseLifeYears_F"), false);
		assetwk.saveEx();
	...
	{
		return
				get_ValueAsInt("DeltaUseLifeYears") <= 0 || get_ValueAsInt("DeltaUseLifeYears_F") <= 0
				|| get_ValueAsInt("DeltaUseLifeYears") != get_ValueAsInt("DeltaUseLifeYears_F") ||
