...
public class FinReport extends SvrProcess
{
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			if (para[i].getParameter() == null)
				...
				p_Org_ID = para[i].getParameterAsString();
		...
		//multiple selection
		//	Optional Org
		if (!Util.isEmpty(p_Org_ID))
			m_parameterWhere.append(" AND AD_Org_ID IN ( ").append(p_Org_ID).append(")");
		//	Optional BPartner
		...
		//	Optional OrgTrx
		if (p_OrgTrx_ID != 0)
			m_parameterWhere.append(" AND AD_OrgTrx_ID=").append(p_OrgTrx_ID);
		
		// ISY-175
		if (p_DateAcct != null)
			m_parameterWhere.append(" AND TRUNC(DateAcct) <= TO_DATE('").append(new SimpleDateFormat("dd/MM/yyyy").format(p_DateAcct)).append("','dd/mm/yyyy')");
	...
	private void insertLine (int line)
	{
		...
		// ISY-175
				int glCategoryID = m_columns[col].get_ValueAsInt("GL_Category_ID");
				if(glCategoryID > 0){
					select.append(" AND GL_Category_ID=").append(glCategoryID);
				}
	...
	private void insertLineSource (int line)
	{
		...
		for (int col = 0; col < m_columns.length; col++)
		{
			insert.append(", ");
			if (listSourceNoTrx)
//				unionInsert.append(", Cast(NULL AS NUMERIC)");
				unionInsert.append(", Cast(NULL AS ").append(numericType).append(")");
				
			//	No calculation
			if (m_columns[col].isColumnTypeCalculation())
			{
//				insert.append("Cast(NULL AS NUMERIC)");
...
//				insert.append("Cast(NULL AS NUMERIC)");
	...
	private MPrintFormat getPrintFormat()
	{
		...
		if (createNew)
		{
//			int AD_Table_ID = TABLE_T_REPORT;		//	T_Report --IDM 11 ambil dari interface
			int AD_Table_ID = I_T_Report.Table_ID;
