...
//							setAD_User_ID(resp.getAD_User_ID());
					...
					else if(resp.isRole())
					{
						...
						if (MSysConfig.getValue(AWNSysConfig.ISY_ALLOW_AUTO_APPROVAL, "N").equals("Y")) {
				...
				if (autoApproval
					...
					&& doc.save())
					return true;	//	done
				if (MSysConfig.getValue(AWNSysConfig.ISY_WF_NODE_ACTION_EMAIL_VALIDATION, "N").equals("N")) {
	...
	private boolean setVariable(String value, int displayType, String textMsg, Trx trx) throws Exception
	{
		...
		if (value == null)
			;
		else if (displayType == DisplayType.YesNo || displayType == DisplayType.Button)
